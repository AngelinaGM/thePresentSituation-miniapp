...
depends_on=None
def upgrade():
    state=sa.Enum("ACTIVE","FROZEN","ARCHIVED",name="wishlist_status")
...
def downgrade():
    op.drop_table("reservations"); op.drop_table("gifts"); op.drop_table("wishlists"); op.drop_table("users"); sa.Enum(name="wishlist_status").drop(op.get_bind())
